...
async function sendFile(params) {
    ...
    if (error)
        return await sendError(error, params);
    ...
    const [ftError, ftype] = await tryToCatch(filetype.fromFile, p.name);
    ...
    if (pipeError)
        await sendError(pipeError, params);
}

/**
 * send error response
 */
async function sendError(error, params) {
    checkParams(params);
...
module.exports._getStatic = getStatic;
async function getStatic(options, request, response) {
    const o = options || {};
    const {root} = options;
    ...
    if (name.indexOf(dir)) {
        return await sendError(Error(`Path ${name} beyond root ${dir}!`), {
            request,
            response,
            name,
        });
